...
export default function RegistrationRecovery({ receipt }: { receipt: RecoveryReceipt }) {
  ...
  useEffect(() => {
    ...
    fetch("/api/registration/recovery", { method: "POST", body: form })
      .then(async response => {
        ...
        if (active) { setStatus(result.paymentStatus); setEntryStatus(result.registrationStatus); }
  ...
  return <section className="mb-6 rounded-2xl border border-green-500/40 bg-zinc-900 p-5" aria-label="Saved registration">
    ...
    <p className="mt-2 text-sm text-gray-300">Do not register again. Keep your private recovery link to return to this entry. Anyone with this link can manage its payment proof—do not share it publicly.</p>
    <div className="mt-4 flex flex-wrap gap-3">
      ...
      {!closed && <button disabled={busy} onClick={() => setMessage("You can leave this page and return using your private recovery link. Payment status is unchanged; the organiser’s payment deadline still applies.")} className="rounded-lg border border-white/30 px-4 py-3">Pay later</button>}
    ...
    {!closed && <div className="mt-4 border-t border-white/10 pt-4">
