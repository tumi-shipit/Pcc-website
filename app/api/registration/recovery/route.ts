...
export async function POST(request: Request) {
  ...
  const { data: entry, error } = await db.from("registrations").select("id,payment_status,registration_status").eq("id", id).single();
  ...
  return Response.json({ paymentStatus: entry.payment_status, registrationStatus: entry.registration_status }, { headers: { "Cache-Control": "no-store" } });
}
