...
export async function POST(request:Request){
  ...
  const {data:products,error:productError}=await supabase.from("store_products").select("id,name,description,category,regular_price,sale_price,sale_starts_at,sale_ends_at,stock_status,stock_quantity,published").in("id",ids).eq("published",true);
  ...
  for(const requestLine of normal){const product=products.find((item)=>item.id===requestLine.productId);const isService=product?.category?.toLowerCase().includes("service")??false;if(!product||product.stock_status!=="available"||(product.stock_quantity!==null&&product.stock_quantity<requestLine.quantity)||(isService&&requestLine.quantity!==1))return Response.json({error:`${product?.name||"A product"} is not available in that quantity.`},{status:409});const unit=activeProductPrice(product);lines.push({product_id:product.id,product_name:product.name,unit_price:unit,quantity:requestLine.quantity,selected_option:requestLine.option,line_total:Number((unit*requestLine.quantity).toFixed(2)),description:product.description,regular_price:product.regular_price});}
  const totalAmount=Number(lines.reduce((sum,line)=>sum+line.line_total,0).toFixed(2)),amountCents=Math.round(totalAmount*100);if(amountCents<200)return Response.json({error:"Online payments require a minimum total of R2.00."},{status:400});
