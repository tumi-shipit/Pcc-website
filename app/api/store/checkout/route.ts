...
type CheckoutRequest={items?:unknown;productId?:unknown;quantity?:unknown;option?:unknown;customerName?:unknown;customerEmail?:unknown;customerPhone?:unknown;fulfillmentMethod?:unknown;deliveryAddress?:unknown};
...
export async function POST(request:Request){
  ...
  if(!await allowRequest(request,"store-checkout",8,600))return rateLimitResponse();
  ...
  const customerName=text(body.customerName,100),customerEmail=text(body.customerEmail,180).toLowerCase(),customerPhone=text(body.customerPhone,40),requestedMethod=text(body.fulfillmentMethod,30),deliveryAddress=text(body.deliveryAddress,500);
  const methods={collection:0,polokwane_delivery:70,paxi_store:150,paxi_home:170} as const;
  const fulfillmentMethod=Object.hasOwn(methods,requestedMethod)?requestedMethod as keyof typeof methods:"collection";
  const deliveryFee=methods[fulfillmentMethod];
  ...
  if(fulfillmentMethod!=="collection"&&deliveryAddress.length<10)return Response.json({error:"Enter the delivery or destination address, including suburb and postal code."},{status:400});
  ...
  const {data:products,error:productError}=await supabase.from("store_products").select("id,name,description,category,regular_price,sale_price,sale_starts_at,sale_ends_at,stock_status,stock_quantity,published,available_options,variant_stock").in("id",ids).eq("published",true);
  ...
  for(const requestLine of normal){const product=products.find((item)=>item.id===requestLine.productId);const isService=product?.category?.toLowerCase().includes("service")??false;const options=(product?.available_options??[]) as string[];const variantStock=(product?.variant_stock??{}) as Record<string,number>;const optionStock=requestLine.option?Number(variantStock[requestLine.option]):Number.NaN;if(!product||product.stock_status!=="available"||(product.stock_quantity!==null&&product.stock_quantity<requestLine.quantity)||(isService&&requestLine.quantity!==1)||(options.length>0&&(!requestLine.option||!options.includes(requestLine.option)))||(options.length===0&&requestLine.option)||(Number.isFinite(optionStock)&&optionStock<requestLine.quantity))return Response.json({error:`${product?.name||"A product"} is not available with that option or quantity.`},{status:409});const unit=activeProductPrice(product);lines.push({product_id:product.id,product_name:product.name,unit_price:unit,quantity:requestLine.quantity,selected_option:requestLine.option,line_total:Number((unit*requestLine.quantity).toFixed(2)),description:product.description,regular_price:product.regular_price});}
  const merchandiseTotal=lines.reduce((sum,line)=>sum+line.line_total,0),totalAmount=Number((merchandiseTotal+deliveryFee).toFixed(2)),amountCents=Math.round(totalAmount*100);if(amountCents<200)return Response.json({error:"Online payments require a minimum total of R2.00."},{status:400});
  ...
  const {error:orderError}=await supabase.from("store_orders").insert({id:orderId,order_number:orderNumber,product_id:first.product_id,product_name:lines.length===1?first.product_name:`${lines.length} store items`,unit_price:merchandiseTotal,quantity:1,total_amount:totalAmount,customer_name:customerName,customer_email:customerEmail,customer_phone:customerPhone,fulfillment_method:fulfillmentMethod,delivery_address:fulfillmentMethod!=="collection"?deliveryAddress:null,delivery_fee:deliveryFee,fulfillment_status:"awaiting_payment"});
  ...
  if(!checkoutResponse.ok||!checkout?.id||!checkout.redirectUrl){await supabase.from("store_orders").update({status:"failed"}).eq("id",orderId);console.error("Yoco checkout creation failed",checkoutResponse.status,checkout?.message);return Response.json({error:"The secure payment could not be started."},{status:502})}
