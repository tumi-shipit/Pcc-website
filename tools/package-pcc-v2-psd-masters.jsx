#target photoshop

(function () {
    app.displayDialogs = DialogModes.NO;
    var root = "C:/Users/Tumelo/Documents/PCC Tournament Polo Catalogue/03 Shirt Mockups/Version 2 Chess Pieces/";
    var sourceDir = root + "PNG Exports/";
    var outDir = root + "PSD Masters/";
    var names = [
        "V2-black-front", "V2-black-back", "V2-white-front", "V2-white-back",
        "V2-black-front-captain", "V2-white-front-captain",
        "V2-black-front-manager", "V2-white-front-manager"
    ];
    for (var i=0; i<names.length; i++) {
        var doc = app.open(new File(sourceDir + names[i] + ".png"));
        doc.activeLayer.name = "00 Final Photorealistic Render";
        var art = doc.layerSets.add(); art.name = "01 Editable Artwork Components";
        art.artLayers.add().name = "PCC Logo & Identification";
        art.artLayers.add().name = "Sleeve Flag";
        art.artLayers.add().name = "Side Panels & Piping";
        art.artLayers.add().name = "Chess Piece Hem Graphic";
        art.artLayers.add().name = "Collar, Cuffs & Shoulder Accent";
        art.visible = false;
        var notes = doc.layerSets.add(); notes.name = "02 Production Notes - Final render above is catalogue source";
        notes.visible = false;
        var options = new PhotoshopSaveOptions();
        options.layers = true; options.embedColorProfile = true;
        doc.saveAs(new File(outDir + names[i] + ".psd"), options, true, Extension.LOWERCASE);
        doc.close(SaveOptions.DONOTSAVECHANGES);
    }
}());
